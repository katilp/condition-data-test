...
# set to accept failure as the cmsRun job is _expected_ to fail (Error: Process completed with exit code 65). This does not help, see below
# set +e
...
./find_db.sh $package $branch /mnt/vol/jec_cfg.py $globaltag
